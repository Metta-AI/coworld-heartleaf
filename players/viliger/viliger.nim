...
proc initBot(name: string, slot: int, decodeVisualSprites = false): Bot =
  ## Builds a new Viliger bot state.
  result = Bot()
...
proc initViewerApp(): ViewerApp =
  ...
  result.window = newWindow(
    title = "Heartleaf Viliger Viewer",
...
proc runBot(
  ...
  ## Connects the Viliger bot to a Heartleaf sprite player endpoint.
  let connectUrl =
    if url.len > 0:
      url
    else:
  ...
  while viewer.viewerOpen():
    try:
      ...
      echo "viliger connected to ", connectUrl
      connected = true
      ...
      connected = false
      echo "viliger reconnecting: ", e.msg
      if gui:
        for i in 0 ..< 25:
          if not viewer.viewerOpen():
            break
