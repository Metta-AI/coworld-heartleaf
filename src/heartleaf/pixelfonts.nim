...
proc loadDigitSprites*(path: string): array[10, Sprite]
  ...
  ## Loads a 10 digit 6x6 sprite strip.
  if not fileExists(path):
    raise newException(IOError, "Missing digit sprite strip: " & path)
  let digits = sliceSpriteStrip(readImage(path), 6, 6, 10)
  if digits.len != 10:
    raise newException(
      IOError,
      "Digit sprite strip must contain 10 digits: " & path
    )
  for i in 0 ..< 10:
    result[i] = digits[i]
...
proc loadLetterSprites*(path: string): seq[Sprite]
    {.raises: [IOError, PixieError].} =
  ## Loads a 6x6 letter and punctuation sprite strip.
  if not fileExists(path):
...
proc letterIndex*(ch: char): int {.raises: [].} =
  ## Returns the old 6x6 letter sprite index for one character.
  if ch >= 'A' and ch <= 'Z':
    return ord(ch) - ord('A')
  if ch >= 'a' and ch <= 'z':
    return ord(ch) - ord('a')
...
proc blitText*(
  ...
  ## Blits old 6x6 letter text into a framebuffer.
  var offsetX = 0
  for ch in text:
    if ch == ' ':
      offsetX += 6
    ...
    if index >= 0 and index < letterSprites.len:
      fb.blitSprite(letterSprites[index], screenX + offsetX, screenY, 0, 0)
    offsetX += 6
...
proc blitText*(
  ...
  ## Blits old 6x6 alphanumeric text into a framebuffer.
  var offsetX = 0
  for ch in text:
    if ch == ' ':
      offsetX += 6
...
proc blitTextTinted*(
  ...
  ## Blits old 6x6 letter text as one palette color.
  var offsetX = 0
  for ch in text:
    if ch == ' ':
      offsetX += 6
...
proc blitTextTinted*(
  ...
  ## Blits old 6x6 alphanumeric text as one palette color.
  var offsetX = 0
  for ch in text:
    if ch == ' ':
      offsetX += 6
