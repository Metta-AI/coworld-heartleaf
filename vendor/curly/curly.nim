  ...
  while true:
    ...
    for request in dequeued:
      if request.easyHandle != nil:
        ...
        discard easyHandle.easy_setopt(cast[libcurl.Option](237), ord(curl.pipeWait))
...
proc newCurly*(maxInFlight = 16, pipeWait = true): Curly =
  ## Creates a new Curly instance that will run up to `maxInFlight` HTTP
  ## requests in parallel. Set `pipeWait` to false to open another connection
  ## instead of waiting for an existing connection to support multiplexing.
